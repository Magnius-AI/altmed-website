...
export function AdminTopbar() {
  ...
  return (
    <div className="sticky top-0 z-20 border-b border-[var(--admin-border)] bg-white px-4 py-2.5">
      <div className="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <h1 className="mt-0.5 text-[1.22rem] font-semibold leading-tight text-neutral-950">{page.title}</h1>
          <p className="mt-0.5 max-w-2xl text-xs leading-5 text-neutral-500">{page.description}</p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <div className="flex h-8 min-w-0 items-center gap-2 rounded-md border border-[var(--admin-border)] bg-[var(--color-bg-gray)] px-3 text-xs text-neutral-500 xl:w-[20rem]">
            <Search className="h-4 w-4 text-slate-400" />
            <span className="truncate">Search content, leads, plans</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex h-8 items-center rounded-md border border-[var(--admin-border)] bg-white px-3 text-xs font-medium text-neutral-700">
              {today}
            </div>
            <button
              type="button"
              className="focus-ring flex h-8 w-8 items-center justify-center rounded-md border border-[var(--admin-border)] bg-white text-neutral-600 transition hover:text-[var(--color-primary)]"
              aria-label="Notifications"
            >
              <Bell className="h-4 w-4" />
            </button>
            <Link href="/" className="btn-primary h-8 px-3 text-xs">
              View Site
